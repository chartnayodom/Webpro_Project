                ...
                <div class="content">
                    <h1 class="title">เช็คระยะรถเบื้องต้น</h1>
                        <div class="field">
                            ...
                            <input class="input" type="text" placeholder="ระยะทาง(กิโลเมตร)" v-model="distance">
                        </div>
                         <div class="field">
                            <label class="label">จำนวนวันต่อสัปดาห์</label>
                            <input class="input" type="text" placeholder="จำนวนวัน" v-model="day">
                        </div>
                        <div class="field">
                            <label class="label">อายุการใช้งาน</label>
                            <input class="input" type="number" placeholder="เดือน" min="1" max="100" v-model="age">
                        </div>
                        <!-- <div class="field">
                           <input type="radio" name="car" value="1" v-model="car">รถยนต์
                           <input type="radio" name="car" value="2" v-model="car">จักรยานยนต์
                        </div> -->
                        <button class="button is-primary"  @click="checkdistance()">คำนวนเบื้องต้น</button>
                        <div class="field p-5">
                           <label class="label"><h2>ผลการเช็คระยะเบื้องต้น</h2></label>
                           <label class="label" v-show="show"><h2>{{checkresult}}</h2></label>
                        </div>
                </div>
            </div>
    </section>
        ...
        checkresult(){
            if(this.age<6){
                if(5000-(this.distance*this.day)>0){
                    this.result = 'เหลือระยะทางอีก '+(5000-(this.distance*this.day))+ ' กิโลเมตร'
                }else{
                ...
                if(10000-(this.distance*this.day)>0){
                    this.result = 'เหลือระยะทางอีก '+(10000-(this.distance*this.day))+ ' กิโลเมตร'
                }else{
                    this.result = 'ควรไปตรวจสอบสภาพรถยนต์'
                }
            }
                ...
                if(40000-(this.distance*this.day)>0){
                    this.result = 'เหลือระยะทางอีก '+(40000-(this.distance*this.day))+ ' กิโลเมตร'
                }else{
                    this.result = 'ควรไปตรวจสอบสภาพรถยนต์'
                }
            }
                ...
                if(60000-(this.distance*this.day)>0){
                    this.result = 'เหลือระยะทางอีก '+(60000-(this.distance*this.day))+ ' กิโลเมตร'
                }else{
                    this.result = 'ควรไปตรวจสอบสภาพรถยนต์'
                }
            }else{
                 if(100000-(this.distance*this.day)>0){
                    this.result = 'เหลือระยะทางอีก '+(100000-(this.distance*this.day))+ ' กิโลเมตร'
                }else{
                    this.result = 'ควรไปตรวจสอบสภาพรถยนต์'
                }
            }
